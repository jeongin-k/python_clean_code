...
class ObserverMock(EventObserver):
    ...
    def notify_unregistration(self, old_observer):
        self.mock.notify_unregistration(old_observer)
...
class TestEventDispatcher(unittest.TestCase):
    ...
    def test_event_with_registration_and_event_with_unregistration(self):
        self.event.type = EventType.IMPORTANT_EVENT
        self.dispatcher.register_observer(EventType.IMPORTANT_EVENT, self.observer)
        self.dispatcher.unregister_observer(EventType.IMPORTANT_EVENT, self.observer)
        self.dispatcher.dispatch_event(self.event, 10)

    def test_register_two_observers_and_unregister_one_observer_result_into_one_call_and_registration_and_unregistration_notification(self):
        with mock.patch.object(self.observer.mock, 'notify') as mock_notify, \
             mock.patch.object(self.observer.mock, 'notify_registration') as mock_registration, \
             mock.patch.object(self.observer.mock, 'notify_unregistration') as mock_unregistration:
            self.event.type = EventType.IMPORTANT_EVENT
            self.dispatcher.register_observer(EventType.IMPORTANT_EVENT, self.observer)
            self.dispatcher.register_observer(EventType.IMPORTANT_EVENT, self.observer2)
            self.dispatcher.unregister_observer(EventType.IMPORTANT_EVENT, self.observer2)
            self.dispatcher.dispatch_event(self.event, 10)

            mock_notify.assert_called_once_with(self.event, 10)
            mock_registration.assert_called_once_with(self.observer2)
            mock_unregistration.assert_called_once_with(self.observer2)
