...
from homeautomation.mocks.spy_light_driver import SpyLightDriver
from homeautomation.mocks.fake_time_service import FakeTimeService
...
class LightSchedulerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.fake_time_service = FakeTimeService()
        self.light_scheduler = LightScheduler(self.fake_time_service, self.light_controller)
        self.scheduled_minute = 1234
        self.light_number = 4

        SpyLightDriver.reset()
        self.light_controller.create()
        SpyLightDriver.add_spies_to_controller(self.light_controller)
    ...
    def check_light_state(self, id, level):
        if id == LightId.UNKNOWN.value:
            self.assertEqual(LightState.UNKNOWN.value, SpyLightDriver.get_last_state())
        else:
            self.assertEqual(level.value, SpyLightDriver.get_state(id))

    def set_time_to(self, day, minute):
        self.fake_time_service.set_day(day.value)
        self.fake_time_service.set_minute(minute)

    def test_create_does_not_change_the_lights(self):
        self.assertEqual(LightId.UNKNOWN.value, SpyLightDriver.get_last_id())
        self.assertEqual(LightState.UNKNOWN.value, SpyLightDriver.get_last_state())
